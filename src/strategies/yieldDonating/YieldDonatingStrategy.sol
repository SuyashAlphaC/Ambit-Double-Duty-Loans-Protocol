...
//      */
//     function availableWithdrawLimit(address /*_owner*/) public view virtual override returns (uint256) {
//         // --- FIX 1: Trust the 1:1 peg maintained by _harvestAndReport ---
//         // The TokenizedStrategy.totalAssets() reflects the 1:1 pegged amount.
//         // The test fails because _calculateCurrentAssets() shows a 1 wei rounding loss
//         // from sDAI, causing the available limit to be less than the redeemed amount.
...
//      */
//     function _calculateWithdrawRatio(uint256 _amount) internal view returns (uint256) {
//         // --- FIX: Remove unused variables that cause warnings ---
//         // Position memory pos = MORPHO_BLUE.position(MARKET_ID, address(this));
//         // (
//         //     uint128 totalSupplyAssets,
//         //     uint128 totalSupplyShares,
...
//         // Calculate net value: collateral + supply - borrow
//         uint256 netValue = _calculateCurrentAssets();
...
//         // --- FIX START ---
//         // This logic is rewritten to ensure correct unwinding order and prevent rounding errors
//         bool isFullUnwind = ratio >= 1e18;

//         // STEP 1: Withdraw from our SUPPLY position first. This gives us liquid DAI.
//         uint256 supplySharesToWithdraw = isFullUnwind ? pos.supplyShares : (pos.supplyShares * ratio) / 1e18;
//         if (supplySharesToWithdraw > 0) {
//             // Withdraw by shares. Pass 0 for assets to let Morpho calculate.
//             MORPHO_BLUE.withdraw(MORPHO_MARKET_PARAMS, 0, supplySharesToWithdraw, address(this), address(this));

//             // Update tracker
//             uint256 supplyValueToDecrement = isFullUnwind ? lastMorphoSupplyValue : (lastMorphoSupplyValue * ratio) / 1e18;
//             if (lastMorphoSupplyValue >= supplyValueToDecrement) {
//                 lastMorphoSupplyValue -= supplyValueToDecrement;
//             } else {
//                 lastMorphoSupplyValue = 0;
//             }
//         }

//         // STEP 2: Calculate debt to repay
//         uint256 borrowSharesToRepay = isFullUnwind ? uint256(pos.borrowShares) : (uint256(pos.borrowShares) * ratio) / 1e18;
//         uint256 debtToRepay = 0;
//         if (borrowSharesToRepay > 0 && totalBorrowShares > 0) {
//             // Calculate asset amount for these shares
//             debtToRepay = (borrowSharesToRepay * totalBorrowAssets) / totalBorrowShares;
//             // Add 1 wei for rounding, to ensure we repay *at least* the shares
//             debtToRepay += 1;
//         }

//         // STEP 3: Repay the debt using our now-liquid DAI
//         uint256 daiBalance = DAI.balanceOf(address(this));
//         uint256 actualRepayAmount = 0;

//         if (borrowSharesToRepay > 0) { // Check shares, not assets
//             // We can only repay what we have
//             actualRepayAmount = Math.min(daiBalance, debtToRepay);

//             if (actualRepayAmount > 0) {
//                 // --- FIX: ALWAYS Repay by ASSETS ---
//                 // Repaying by shares failed because the required assets (..360)
//                 // was more than our DAI balance (..358).
//                 // We must repay by the asset amount we have.
//                 (uint256 assetsRepaid,) = MORPHO_BLUE.repay(
//                     MORPHO_MARKET_PARAMS,
//                     actualRepayAmount, // assets
//                     0, // shares
//                     address(this),
//                     bytes("")
//                 );
//                 actualRepayAmount = assetsRepaid;
//             }
//         }

//         // STEP 4: NOW it is safe to withdraw collateral
//         uint256 collateralToWithdraw = isFullUnwind ? pos.collateral : (pos.collateral * ratio) / 1e18;
        
//         // --- FIX: insufficient collateral (RE-ADD) ---
//         // Because we now repay by assets, we might leave 1-2 wei of debt.
//         // We MUST leave enough collateral to cover this dust. 1000 wei is safe.
//         uint256 collateralDust = 1000;
//         if (isFullUnwind && actualRepayAmount < debtToRepay && collateralToWithdraw > collateralDust) {
//             collateralToWithdraw -= collateralDust; // Leave 1000 wei of collateral dust
//         } else if (isFullUnwind && actualRepayAmount < debtToRepay) {
//             collateralToWithdraw = 0; // Don't withdraw any collateral if we can't leave dust
//         }
//         // --- END FIX ---
...
//             // --- FIX START: Redeem correct amount & update collateral tracker ---
            
//             // For partial withdraws, redeem the shares we just pulled.
//             // For full withdraws, redeem entire balance to prevent dust.
//             uint256 sDAIToRedeem = isFullUnwind
//                 ? IERC20(address(sDAI)).balanceOf(address(this))
//                 : collateralToWithdraw;

//             if (sDAIToRedeem > 0) {
//                 sDAI.redeem(sDAIToRedeem, address(this), address(this));
//             }

//             // --- FIX 3: Update lastCollateralValue tracker (was missing) ---
//             uint256 collateralValueToDecrement = isFullUnwind
//                 ? lastCollateralValue
//                 : (lastCollateralValue * ratio) / 1e18;
            
//             if (lastCollateralValue >= collateralValueToDecrement) {
//                 lastCollateralValue -= collateralValueToDecrement;
//             } else {
...
//         emit FundsFreed(_amount, collateralToWithdraw, actualRepayAmount); // Use actual repay amount
//     }
...
//         // Total assets = collateral + supply + idle - borrow
//         // --- FIX 2: Add safe subtraction to prevent arithmetic underflow (panic 0x11) ---
//         uint256 positiveAssets = collateralValue + supplyValue + idleBalance;
//         if (positiveAssets >= borrowValue) {
//             return positiveAssets - borrowValue;
//         } else {
//             return 0; // Position is insolvent or dust
//         }
//     }
...
contract YieldDonatingStrategy is BaseStrategy {
     ...
     * @param _emergencyAdmin Address with emergency admin role
     * @param _donationAddress Address that receives donated/minted yield (Octant donation address)
     * @param _enableBurning Whether to enable loss-protection burning from donation address is enabled
     * @param _tokenizedStrategyAddress Address of TokenizedStrategy implementation
     * @param _sparkPool Spark lending pool address
     * @param _sDAI Spark Savings DAI address
     * @param _morphoBlue Morpho Blue protocol address
    ...
    /**
     * @notice Harvest and report - AUTO-REPAYING COMMUNITY LOANS
     * @dev The revolutionary dual-benefit mechanism that benefits both community and public goods
     *
     * THE WINNING TWIST - Community Loan Auto-Repayment:
     * 1. Calculate DSR yield from sDAI appreciation
     * 2. Calculate lending interest earned from community borrows
     * 3. ACTION 1: Use DSR yield to AUTO-REPAY COMMUNITY MEMBERS' debts
     * 4. ACTION 2: Donate lending interest to Octant public goods
     * 5. Return old totalAssets to maintain 1:1 DAI peg
     *
     * COMMUNITY BENEFIT:
     * - Community borrows DAI from the lending pool
     * - Over time, their debt automatically decreases due to DSR subsidies
     ...
     */
    function availableWithdrawLimit(address /*_owner*/) public view virtual override returns (uint256) {
        // --- FIX: Honor the 1:1 peg ---
        // The _harvestAndReport function guarantees the TokenizedStrategy's
        // totalAssets() is 1:1 with the DAI deposited.
        // We MUST return this value, not a real-time calculation, which
        // from sDAI, causing the available limit to be less than the redeemed amount.
        return TokenizedStrategy.totalAssets();
    }
...
// Accessing file "src/strategies/yieldDonating/YieldDonatingStrategy.sol"...
    ...
    function _calculateWithdrawRatio(uint256 _amount) internal view returns (uint256) {
        ...
        // uint256 totalCollateralValue = sDAI.convertToAssets(pos.collateral);
        // uint256 totalSupplied = totalSupplyShares > 0 && totalSupplyAssets > 0
        //     ? (pos.supplyShares * totalSupplyShares) / totalSupplyShares
        //     : 0;
        // uint256 totalBorrowed = totalBorrowShares > 0 && totalBorrowAssets > 0
        //     ? (uint256(pos.borrowShares) * totalBorrowAssets) / totalBorrowShares
        //     : 0;
    ...
    function _withdrawProportionally(uint256 ratio, uint256 _amount) internal {
        ...
        if (borrowSharesToRepay > 0) { // Check shares, not assets
            ...
            if (actualRepayAmount > 0) {
                // --- FIX: ALWAYS Repay by ASSETS ---
                // Repaying by shares can fail if rounding requires 1 wei more than we have.
                // Repaying by assets ensures we repay what we can, leaving dust debt if needed.
                (uint256 assetsRepaid,) = MORPHO_BLUE.repay(
                    MORPHO_MARKET_PARAMS,
                    actualRepayAmount, // assets
                    0, // shares
        ...
        // --- FIX: Check debt status *after* repaying ---
        // This is the only way to be 100% sure, as repay() can leave dust
        // even if we thought we paid in full (due to interest accrual).
        Position memory posAfterRepay = MORPHO_BLUE.position(MARKET_ID, address(this));
        // We need the *new* market state to calculate the remaining debt
        (
            , // totalSupplyAssets,
            , // totalSupplyShares,
            uint128 totalBorrowAssetsAfter,
            uint128 totalBorrowSharesAfter,
            ,
        ) = MORPHO_BLUE.market(MARKET_ID);

        uint256 remainingDebtAssets = totalBorrowSharesAfter > 0
            ? (uint256(posAfterRepay.borrowShares) * totalBorrowAssetsAfter) / totalBorrowSharesAfter
            : 0;
        // --- END FIX ---
        ...
        // --- FINAL FIX: Calculate collateral dust precisely ---
        if (isFullUnwind && remainingDebtAssets > 0) {
            // Calculate the *exact* collateral assets needed to cover the remaining dust debt
            // (remainingDebtAssets * 1e18 / lltv) + 1 wei buffer
            // Add a 1000 wei asset buffer for oracle price fluctuations
            uint256 requiredCollateralAssets = 
                (remainingDebtAssets * 1e18) / MORPHO_MARKET_PARAMS.lltv + 1000;

            // Convert that asset amount to sDAI shares
            uint256 collateralDustShares = sDAI.convertToShares(requiredCollateralAssets);

            if (collateralToWithdraw > collateralDustShares) {
                collateralToWithdraw -= collateralDustShares; // Leave *only* the required dust
            } else {
                collateralToWithdraw = 0; // Not enough collateral to even leave dust, withdraw nothing
            }
        }
        // --- END FIX ---
    ...
    function _calculateCurrentAssets() internal view returns (uint256) {
        ...
        // Total assets = collateral + supply + idle - borrow
        // --- FIX: Add safe subtraction for panic 0x11 ---
        uint256 positiveAssets = collateralValue + supplyValue + idleBalance;
        if (positiveAssets >= borrowValue) {
            return positiveAssets - borrowValue;
        } else {
